...
std::vector<Vec2> generatePolygon(int sides, float sideLength, Vec2 center) {
    if (sides < 3) return {};
    ...
    float circumradius = sideLength / (2.0f * std::sin(M_PI / (float)sides));

    // determine the starting angle 
    float angleStep = (2.0f * M_PI) / (float)sides;
    float currentAngle = 0.0f; 

    for (int i = 0; i < sides; ++i) {
        float vx = center.first + circumradius * std::cos(currentAngle);
        float vy = center.second + circumradius * std::sin(currentAngle);
...
static int l_draw_polygon(lua_State* L){
    ...
    std::cout << "Debug::draw_polygon called with " << lua_gettop(L) << " arguments." << std::endl;
...
void bind_debug(lua_State* L){
    static const luaL_Reg debug_lib[] = {
        ...
        {"draw_polygon", l_draw_custom_shape},
        {NULL, NULL}
    };
