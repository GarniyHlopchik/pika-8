...
int PriorityQueue::get_id(){
    static int id = 0;
    return id++;
}
void PriorityQueue::enqueue(TypeOfData data, int priority){
    QueneNode node;
...
void PriorityQueue::bubble_up_max(int index){
    if(index<3){
        return;
    }
...
void PriorityQueue::bubble_up_min(int index){
    if(index<3){
        return;
    }
    int gramps = (((index - 1) / 2) - 1) / 2;
...
void PriorityQueue::remove_and_correct(int index) {
...
void PriorityQueue::push_down_min(int index) {
    int size = tree.size();
...
void PriorityQueue::push_down_max(int index) {
    int size = tree.size();
    if (2 * index + 1 >= size) return;
...
QueueNode PriorityQueue::dequeue(QueueProtocol protocol){
    QueueNode node;
...
QueueNode PriorityQueue::peek(QueueProtocol protocol){
    QueueNode node;
    if(protocol==QueueProtocol::HIGHEST_PRIORITY){
